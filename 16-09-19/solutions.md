...
We scan the array from right to left: each time a new maximum is found, we store it in a vector and finally print this one in reverse order. Complexity is O($n$s).
...
The idea is to use memoization in order to avoid a O($n^2$) solution. We thus use a queue as an auxiliary data structure and we scan the array. At each iteration, we remove from the queue (which is initially empty) the elements which are no longer in the window and those being smaller than the current one. Then we push the current element: the current maximum is thus stored in the queue front.